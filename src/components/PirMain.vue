...
export default class PirMain extends Vue {
  ...
  async rowclick(entityData : any): Promise<void> {
    ...
    let model = this.$deepCopy(this.entityTypeModelDown) as VisualTreeNodePathModel[]
  ...
  async rowClickDown(entityData:any){
    this.buttonState = true
    console.log("глубина 1",entityData);
    await this.getNode(this.nodePath).then(this.pushToModel)
    let model = this.entityTypeModel.find(t=>t.EntityId== entityData.EntityId)
    if(model!=null){
      this.entityTypeModelDown = this.$deepCopy(this.nodePath) as VisualTreeNodePathModel[]
      this.entityTypeModelDown.push(new VisualTreeNodePathModel(model.NodeId,model.EntityId,model.ParentNodeId,null , null))
    }
  }
  ...
  getMasterEntityModel() {
    ...
    if (this.localEntityId > 0) {
      if (this.treeId && this.localNodePath) {
        this.entityController.ProtectedGetMasterModel(this.localEntityTypeId, this.localEntityId, this.treeId, this.entityTypeModelDown)
            .then((response) => {this.gotMasterEntityModel(response);})
            .catch(error => {this.showError(error);
            });
      } else {
        this.visualTreeController.GetMasterModelForNodeId(this.localEntityTypeId, this.treeId, this.entityTypeModelDown, this.entityTypeNode.NodeId)
